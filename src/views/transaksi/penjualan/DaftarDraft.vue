                ...
                <span v-if="props.column.field === 'action'">
                  <span>
                    ...
                    >
                      ...
                      <b-dropdown-item @click="view(props.row.nomor)">
                        <feather-icon
                          icon="EyeIcon"
                          class="mr-50"
                        />
...
export default {
  ...
  created() {
    this.rows = store.getters['app-transaksi/getListDraftPenjualan']
  ...
  methods: {
    del(index) {
      this.$swal({
        ...
        if (result.value) {
          store.commit('app-transaksi/REMOVE_DRAFT_PENJUALAN', index)
          this.$swal({
            icon: 'success',
            title: 'Deleted!',
            customClass: {
    ...
    view(index) {
      router.push({ name: 'transaksi-penjualan-tambah', params: { id: index } })
    },
    jumlahItem(obj) {
