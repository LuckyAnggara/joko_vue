...
export default {
  ...
  watch: {
    ...
    searchQuery(query) {
      if (query === '') {
        this.dataKendaraanDinas = this.dataTemp
      } else {
        this.dataKendaraanDinas = this.dataTemp.filter(
          item => item.nama.toLowerCase().indexOf(query) > -1 || item.bidang.nama.toLowerCase().indexOf(query) > -1,
        )
      }
    },
    /* eslint-enable */
    bidangFilter(x) {
      if (x.id === '' || x.id === null || x.id === 0) {
        ...
        this.dataKendaraanDinas = this.dataTemp.filter(item => item.bidang.id === x.id)
      }
    },
    tahun() {
      this.loadMak()
  ...
  computed: {
    ...
    totalData() {
      return this.dataKendaraanDinas.length
    },
  },
  methods: {
    loadBarang() {
      this.$store.dispatch('app-bmn/fetchKenderaanDinas').then(res => {
        this.$store.commit('app-bmn/SET_KENDARAAN_DINAS', res.data)
  ...
  setup() {
    ...
    const isBusy = false
    const dataKendaraanDinas = ref([])
    const dataTemp = ref([])
    const tableColumns = [
      { key: 'id', label: '#' },
      { key: 'nama', label: 'NAMA BMN' },
      { key: 'jenis' },
