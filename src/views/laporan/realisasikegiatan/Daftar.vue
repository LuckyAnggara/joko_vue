  ...
  <section>
    <b-card>
      ...
      >
        ...
        <template #cell(realisasi_kegiatan)="data">
          ...
          <span>
            {{
              data.item.jenis === 'PERJADIN'
                ...
                : $moment(data.item.tanggal_realisasi_kegiatan).format('D MMMM Y')
            }}
          </span>
        </template>
        <template #cell(realisasi)="data">
          <span> {{ formatRupiah(data.item.total_realisasi) }} </span>
        </template>
  ...
  methods: {
    ...
    loadData() {
      if (this.tahun !== null) {
        ...
        this.$store
          .dispatch('app-laporan/fetchLaporanRealisasiKegiatan', {
